...
```
Настройте переменные окружения - в каталоге проекта создайте файл .env, указав в нем:
```sh
YANDEX_KEY=insert_your_key
```
Вместо insert_your_key вы должны указать API ключ, который можете получить в [кабинете разработчика](https://developer.tech.yandex.ru/).
Данный ключ дает возможность программе поучать координаты ресторанов и заказчиков, для вычисления расстояния между ними.
